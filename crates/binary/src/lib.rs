...
// #[cfg(test)]
// mod test {
//     use humm_provenance_core as core;
...
//         let content = core::content::Content::from(b"foo".to_vec());
//         let version = core::version::Version::Zero;
//
...
//         let ecosystem = core::roughtime::ecosystem::Ecosystem::try_from(
//             core::roughtime::ecosystem::env::EcosystemJsonFilePath,
//         )
//         .unwrap();
//
...
//         let device_sends_pub_keys_on_wire = || -> (core::device::keys::PubKeyPortable, core::roughtime::ecosystem::server::public_key::KeysPortable) {
...
//         // of strings
//         let idp_sends_back_signed_jwt =
//             |device_pub_portable: core::device::keys::PubKeyPortable,
//              time_pubs_portable: core::roughtime::ecosystem::server::public_key::KeysPortable|
//              -> Option<core::jwt::token::Token> {
//                 let device_pub: core::device::keys::PubKey =
//                     (&device_pub_portable).try_into().unwrap();
//                 let time_pubs: core::roughtime::ecosystem::server::public_key::Keys =
//                     (&time_pubs_portable).try_into().unwrap();
//
...
//             };
//
//         let device_signs_content_and_jwt = |version: &core::version::Version,
//                                             content: &core::content::Content,
//                                             jwt_token: &core::jwt::token::Token|
//          -> Option<core::device::signature::Signature> {
...
//             let version_int: core::version::VersionInt = version.into();
//
...
//             let content_hash: core::content::Hash = content.into();
//
...
//             let jwt_signature: core::jwt::token::Signature = jwt_token.try_into().unwrap();
//
//             Some(
//                 (&core::device::signature::SignatureInput {
...
//
//         let idp_signed_jwt_portable =
//             idp_sends_back_signed_jwt(device_pub_key_portable, time_pub_keys_portable).unwrap();
//
//         let device_signature =
//             device_signs_content_and_jwt(&version, &content, &idp_signed_jwt_portable).unwrap();
//
//         println!(
//             "zzz {:?}",
//             serde_json::to_string(&device_signature).unwrap()
//         );
//
...
//         //
//
//         let roughtime: core::roughtime::Roughtime = core::roughtime::RoughtimeInput {
...
//         .unwrap();
//
//         let provenance = core::Provenance {
//             version,
